...
3.2.Validation Checks
    ...
    • Validate that both user_id,transaction_id  and  device_id have a minimum length of 1 character
...
3.3. Duplicate Transaction Check
    ...
    • If a duplicate transaction is found, return:
                   "Transaction ID already exists."

3.4. Fraud Rule Evaluation:
	If the transaction is not a duplicate, evaluate it based on the following rules:
	Rule 1: High Risk
	    • If the transaction amount is greater than 2000, mark it as High Risk.
	Rule 2: Suspicious Activity
	    • If a user performs more than three transactions, mark the transaction as Suspicious.
...
Make user input that has the following columns 
    • transaction_id 
    • user_id
    • amount
    • timestamp
...
4.2.1 List all transactions 
    • columns: Transaction ID, User, Amount, Risk, Rule

4.2.2 Highlight flagged ones
    • high-risk-row -> background-color: #ffcccc; 
    • suspicious-row -> background-color: #fff3cd;
    • normal-row -> background-color: #eafaf1;
...
6. To handle 1 million transactions per day:
